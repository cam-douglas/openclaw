...
- **Workspace projects on the droplet (for example Moonshot Engine):** A **separate** project living under the OpenClaw workspace has its own **project root** `.env` next to that project’s `AGENTS.md` (gitignored). With the default workspace for user `root` (`~/.openclaw/workspace`), the Moonshot Engine env file is **`/root/.openclaw/workspace/moonshot-engine/.env`**. Agents working **inside that project** should read keys from that path (or `<configured-workspace>/moonshot-engine/.env` if `agents.defaults.workspace` is overridden), not from `~/.config/...` first and not from `~/.openclaw/.env` on the droplet. The sync script **removes** `/root/.openclaw/.env` on purpose; that is not the canonical file for a named project’s app keys.
