...
- Context limits: bundled **CLI backends** (**`claude-cli`**, **`google-gemini-cli`**, **`codex-cli`**) now resolve context windows via the matching **API provider** (Anthropic, Google, OpenAI) for every model id on that backend. Discovery previously stored **~16k** (per-response output) under qualified CLI ids, which matched persisted session caps and blocked stale detection, so **`status`** / TUI could keep showing **`?/16k`** after reset.
