...
- Droplet: optional **`scripts/droplet-gateway-watchdog.sh`** (listener + **`/healthz`** + **`/readyz`**, no `openclaw`/LLM calls, **`tailscaled`** check, rate-limited **`systemctl --user restart`**), **`openclaw-gateway-watchdog.timer`** install path, **`scripts/droplet-ufw-ssh-tailscale-only.sh`**, **`DROPLET_SSH_HOST`**, and optional **`OPENCLAW_DROPLET_SSH_IDENTITY`** / **`OPENCLAW_DROPLET_SSH_IDENTITY_AGENT_NONE`** (DigitalOcean doc updates).
