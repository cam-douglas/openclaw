...
- Droplet: optional **`scripts/droplet-gateway-watchdog.sh`** (listener + **`/healthz`** + **`/readyz`**, no `openclaw`/LLM calls, **`tailscaled`** check, rate-limited **`systemctl --user restart`**), **`scripts/install-droplet-watchdog.sh`** (enables **`openclaw-gateway-watchdog.timer`**, removes legacy cron), **`scripts/droplet-ufw-ssh-tailscale-only.sh`**, **`DROPLET_SSH_HOST`**, and optional **`OPENCLAW_DROPLET_SSH_IDENTITY`** / **`OPENCLAW_DROPLET_SSH_IDENTITY_AGENT_NONE`** (DigitalOcean doc updates).
...
- TUI: pairing disconnect hint now points to **`openclaw devices approve --latest`** (with **`openclaw devices list`** + **`openclaw devices approve <requestId>`** as the explicit alternative).
- TUI/sessions: **`sessions.json` `contextTokens`** and **`agents.defaults.contextTokens`** in the **16k/32k/64k** output-cap class are **never** used as the footer denominator (they were still shown when catalog resolution landed in an awkward middle band that failed the old stale heuristic). Gateway session rows and defaults apply a final mistaken-cap sanitize so **`?/16k`** cannot leak from the store alone.
...
- Context limits: bundled **CLI backends** (**`claude-cli`**, **`google-gemini-cli`**, **`codex-cli`**) resolve context via qualified CLI discovery when sane, else via the **API provider** catalog, while ignoring mistaken **`models.providers`** rows for those CLI ids that echo **~16k/32k/64k** output caps (see **`docs/concepts/compaction.md`**). This fixes **`?/16k`** when a bad catalog row or duplicate discovery value was returned before the Anthropic/Google/OpenAI window.
- Gateway sessions: session defaults and stale **`sessions.json` `contextTokens`** now treat **16k/32k/64k** as bogus when the catalog window cannot be resolved (**`undefined`**), and **`getSessionDefaults`** uses **`resolveContextTokensForModel`** so TUI/footer defaults are not stuck on **`16k`** from **`agents.defaults.contextTokens`** or lookup alone.
- macOS/droplet: **`openclaw … droplet`** skips the SSH-exit completion chime by default when the remote command is **`tui`** (SSH stays open until you quit). Use **local** **`openclaw tui`** against the same gateway for the per-reply chime, or set **`OPENCLAW_DROPLET_SSH_EXIT_CHIME=1`** to restore chime when the SSH session ends.
- Agents/context: the embedded tool-result guard no longer treats **`model.maxTokens`** (per-response output cap, often ~16k) as the **context window**, which could show **>200%** usage and compact far too early vs Claude’s **~200k** API context; it now uses the same resolved context window as the rest of OpenClaw (`resolveContextWindowInfo`). When **`toolResultContextHeadroomRatio`** is unset, merged defaults use a **lower** headroom than the legacy **~0.95** behavior so preemptive tool compaction runs sooner on long contexts; the post-tool overflow high-water mark remains **~98%** so it stays above the tool budget and can still trigger session compaction when non-tool content dominates.
...
- CLI/droplet: forward `OPENCLAW_GATEWAY_TOKEN` / `OPENCLAW_GATEWAY_PASSWORD` from the local environment into the remote SSH session so `openclaw tui droplet` and similar commands can authenticate to the VPS gateway without HTTP 401; wrap the remote `bash --noprofile --norc` invocation in `env -i` with a minimal PATH/HOME/TERM so inherited sshd noise (for example `BASH_ENV` or environment dumps) does not flood the TUI; also resolve those keys from standard `.env` files with **last-wins** order (`$PWD/.env` → `~/openclaw/.env` → `~/.openclaw/.env` → `OPENCLAW_ENV_FILE`) so `~/.openclaw/.env` can override a repo `.env` after `loadCliDotEnv`.
- TUI/gateway: prefer `OPENCLAW_GATEWAY_TOKEN` over `gateway.auth.token` when both are set (matches gateway CLI env-first behavior) so a forwarded token from `openclaw … droplet` is not overridden by a stale token in the remote config.
- Config: emit the “newer OpenClaw” config warning at most **once per process** so short-lived config cache reloads (for example in the TUI) do not flood stderr.
- TUI/gateway: when resolving password auth without `gateway.auth.mode`, prefer `OPENCLAW_GATEWAY_PASSWORD` over `gateway.auth.password` when both are set (env-first).
