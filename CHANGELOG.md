...
- Context limits: **`claude-cli`** models (for example **`claude-cli/claude-sonnet-4-6`**) now resolve context windows like **Anthropic API** Claude models. Discovery previously stored **~16k** (per-response output) under **`claude-cli/...`**, which matched persisted session caps and blocked stale detection, so **`status`** / TUI could keep showing **`?/16k`** after reset.
